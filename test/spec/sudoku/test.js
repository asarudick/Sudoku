	...
	describe( 'Sudoku Test Suite', function () {
		...
		describe( 'getRow', function () {
			it( 'should get the first row', function () {
				var row = sudoku.getRow( 0 );
				return expect( row.map( function ( a ) {
						return a.answer;
					} ) )
		...
		describe( 'getColumn', function () {
			it( 'should get the first column', function () {
				var column = sudoku.getColumn( 0 );
				return expect( column.map( function ( a ) {
						return a.answer;
					} ) )
					.to.eql( [ 1, 7, 4, 2, 8, 5, 3, 9, 6 ] );
			} );
		} );

		describe( 'getBox', function () {
			it( 'should get the first box', function () {
				var box = sudoku.getBox( 0 );
				return expect( box.map( function ( a ) {
						return a.answer;
					} ) )
					.to.eql( [ 1, 2, 3, 7, 8, 9, 4, 5, 6 ] );
			} );
		} );

		describe( 'isValidBoard', function () {
			it( 'should be valid', function () {
				return expect( sudoku.isValidBoard() )
					.to.equal( true );
			} );
		} );
